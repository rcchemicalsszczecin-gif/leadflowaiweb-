...
for(const required of ["pracują jak produkt","Wyceń projekt","Zobacz realizacje","v14-browser","v14-phone","Sześć warstw jednego produktu cyfrowego","Jedna marka. Trzy urządzenia","LIQUID WEB CONSTRUCTOR","Z płynnej powierzchni","SEO · AEO · GEO","RAG · encje · integracje"])if(!home.includes(required))fail(`V14 homepage artifact missing: ${required}`);
...
console.log(`PUBLIC_ARTIFACT_V14_PASS html=${htmlFiles.length} placeholders=ABSENT retired-en=ABSENT buyer-first=PASS services=PASS devices=PASS liquid=PASS motherboard=ABSENT css=CONSOLIDATED trust=PASS`);
